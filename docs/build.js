...
const pageCatalog = {
  en: {
    ...
    panelConfigurator: 'Go runtime with GUI + TUI',
    ...
    featureNetworkTitle: 'Local runtime with GUI/TUI',
    featureNetworkBody:
    ...
    architectureBody:
      'The system separates the hardware-facing bridge from the Go runtime and the separate browser-push app. The Go runtime includes both a desktop GUI and a terminal TUI for local provisioning and status checks, while the Pico reads burner data and streams signed telemetry to a Go process on the same trusted local network. That Go process exposes the API and USB configuration flow while the bridge stays focused on measurement, ingest, and alert delivery.',
    stack1: 'Burner serial data',
    stack2: 'Pico W / Pico 2 W firmware',
    stack3: 'Signed TCP ingest',
    stack4: 'Go runtime with GUI + TUI + local APIs',
