...
static tcp_state_t s_state = TCP_STATE_IDLE;
...
[[nodiscard]] static bool queue_push(const uint8_t *data, size_t len) {
	...
	if (s_queue_count >= TELEMETRY_QUEUE_LEN) {
		...
		s_queue_count--;
		printf("tcp_client: telemetry queue full, dropping oldest frame (%u bytes)\n",
...
static void flush_queue(void) {
	while (s_state == TCP_STATE_CONNECTED && s_pcb != NULL && s_queue_count > 0) {
		...
		printf("tcp_client: tcp_write failed (%d)\n", (int)err);
		abort_and_retry();
		return;
	}
}
...
void tcp_client_init(const char *host, uint16_t port, const char *device_key) {
	...
	if (server_target_ready() && s_device_key[0] != '\0') {
		...
		printf("tcp_client: telemetry key missing – provision DEVICEKEY via USB\n");
	}
}
...
void tcp_client_send_data(const vikingbio_data_t *data) {
	...
	if (!build_frame(data, frame, &frame_len)) {
		printf("tcp_client: failed to build telemetry frame\n");
		return;
	}
	if (!queue_push(frame, frame_len)) {
		printf("tcp_client: failed to queue telemetry frame\n");
		return;
	}
...
void tcp_client_poll(void) {
	if ((s_state == TCP_STATE_RESOLVING || s_state == TCP_STATE_CONNECTING ||
		 (s_state == TCP_STATE_CONNECTED && s_queue_count > 0)) &&
		time_reached(s_timeout)) {
		printf("tcp_client: timeout – reconnecting\n");
		abort_and_retry();
	}

	if (s_state == TCP_STATE_RETRY_WAIT && time_reached(s_retry_time)) {
		s_state = TCP_STATE_IDLE;
	}
	...
	if (s_state == TCP_STATE_IDLE) {
		start_connect();
		return;
	}
	if (s_state == TCP_STATE_CONNECTED) {
		flush_queue();
	}
}

bool tcp_client_is_active(void) {
	return s_state == TCP_STATE_RESOLVING || s_state == TCP_STATE_CONNECTING ||
		   s_state == TCP_STATE_CONNECTED;
}
